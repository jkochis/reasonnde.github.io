...
function(
	...
	var Testimonial = WidgetView.extend({

		initialize: function() {
			this.setup();
			this.registerMediaHelper(TestimonialSmallAndAbove);
			this.registerMediaHelper(TestimonialXSmall);
		},

		defaults: {
			testimonialDuration: 0.3,
			testimonialEase: 'Expo.easeInOut'
		},

		events: {
			'click .read-testimonial': 'toggleTestimonial',
			'click .close-testimonial': 'toggleTestimonial',
			'mouseenter .read-testimonial': 'onReadTestimonialEnter',
			'mouseleave .read-testimonial': 'onReadTestimonialLeave'
		},
			...
			if(this.$slides.find('.slide').length > 1) {
				...
				this.carousel = new Carousel({
					...
					onCycleCreated: function() {
						...
						// ADDENDUM EVERWELL
					/*	var arrows = that.$el.find('.arrow');
						arrows.on('click', function() {
							var arrowTarget = $(event.currentTarget);
							if(arrowTarget.hasClass('arrow-left')) {
								that.$slides.cycle('prev');
								//that.$slides.cycle('pause');
							}
							if(arrowTarget.hasClass('arrow-right')) {
		...
		onReadTestimonialEnter: function(ev) {
			TweenMax.fromTo($(ev.currentTarget).find('.icon'), 0.15, {'top': 0}, {'top': -32, ease: "Quad.easeOut"});
		},

		onReadTestimonialLeave: function(ev) {
			TweenMax.fromTo($(ev.currentTarget).find('.icon'), 0.15, {'top': -32}, {'top': -64, ease: "Quad.easeOut"});
		},

		toggleTestimonial: function(ev) {
			var that = this;
...
var TestimonialSmallAndAbove = MediaHelper.extend({
	...
	var TestimonialXSmall = MediaHelper.extend({
